...
function listPostsHelper(posts, options) {
    ...
    if (style === 'list') {
        ...
        posts.forEach(post => {
            ...
            result += `<a class="${className}-list-link" href="${hexo_util_1.url_for.call(this, post.path)}">`;
        ...
        posts.forEach((post, i) => {
            if (i)
                result += separator;
            const title = post.title || post.slug;
            result += `<a class="${className}-link" href="${hexo_util_1.url_for.call(this, post.path)}">`;
            result += transform ? transform(title) : title;
            result += '</a>';
        });
    }
...
//# sourceMappingURL=list_posts.js.map